...
foreach ($entry in $x)
{
    $Filename = $entry.P2.Replace(" ", "_") + ".txt"
    ...
    Add-Content -Value "Teilnehmer-Domain:`t$($entry.p4)" -Path $OutputFile -Encoding UTF8 -WhatIf:$false -Confirm:$false
    Add-Content -Value "Custom-Domain:`t`t$($entry.p5)`n" -Path $OutputFile -Encoding UTF8 -WhatIf:$false -Confirm:$false
    ...
    Add-Content -Value "Tenantadmin PW:`t`t$($entry.p8)`n" -Path $OutputFile -Encoding UTF8 -WhatIf:$false -Confirm:$false
    Add-Content -Value "Azure DNS User:`t`t$($entry.p11)" -Path $OutputFile -Encoding UTF8 -WhatIf:$false -Confirm:$false
    Add-Content -Value "Azure DNS User PW:`t$($entry.p12)`n" -Path $OutputFile -Encoding UTF8 -WhatIf:$false -Confirm:$false
    Add-Content -Value "IP1:`t`t`t$($entry.p9)" -Path $OutputFile -Encoding UTF8 -WhatIf:$false -Confirm:$false
    Add-Content -Value "IP2:`t`t`t$($entry.p10)" -Path $OutputFile -Encoding UTF8 -WhatIf:$false -Confirm:$false
