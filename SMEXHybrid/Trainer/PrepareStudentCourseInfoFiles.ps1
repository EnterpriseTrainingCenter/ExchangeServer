...
# Import Excel File
$x = Import-Excel -Path $Path.FullName -StartRow 5 -StartColumn 1 -EndColumn 11 -EndRow (4 + $NumberofStudents) -NoHeader

foreach ($entry in $x)
{
    $Filename = $entry.P2.ToString() + ".txt"
    ...
    Add-Content -Value "Teilnehmer-Domain:`t$($entry.p3)" -Path $OutputFile -Encoding UTF8 -WhatIf:$false -Confirm:$false
    Add-Content -Value "Custom-Domain:`t`t$($entry.p3)$($entry.P4)`n" -Path $OutputFile -Encoding UTF8 -WhatIf:$false -Confirm:$false
    ...
    Add-Content -Value "Subnet Mask:`t255.255.255.192" -Path $OutputFile -Encoding UTF8 -WhatIf:$false -Confirm:$false
